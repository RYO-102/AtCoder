...
int main(){
	...
	vvll appear(10, vll(10, 0));
	rep(i, 0, N){
		rep(j, 0, 10){
			appear.at(S.at(i).at(j) - '0').at(j)++;
		}
	}
	...
	rep(i, 0, 10){
		reverse(all(appear.at(i)));
		auto pos = max_element(all(appear.at(i)));

		chmin(ans, 9 - (pos - appear.at(i).begin()) + (*pos - 1) * 10);
	}
