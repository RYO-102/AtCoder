...
int main(){
	...
	map<char, ll> arrangement;
	rep(i, 0, 26) arrangement[X.at(i)] = i;

	sort(all(S), [&](string a, string b){
		rep(i, 0, min(a.length(), b.length())){
			if(arrangement[a.at(i)] != arrangement[b.at(i)]){
				return arrangement[a.at(i)] < arrangement[b.at(i)];
			}
		}
