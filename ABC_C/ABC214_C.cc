...
int main(){
	...
	ll first = min_element(all(T)) - T.begin();
	vll ans(N);

	ans.at(first) = T.at(first);
	
	rep(i, 1, N) ans.at((first + i) % N) = min(ans.at((first + i - 1) % N) + S.at((first + i - 1) % N), T.at((first + i) % N));

	rep(i, 0, N) cout << ans.at(i) << endl;
