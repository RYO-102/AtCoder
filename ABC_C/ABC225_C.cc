...
int main(){
	...
	bool vertical = true, width = true;

	rep(i, 0, M){
		rep(j, 0, N - 1) if(B.at(j + 1).at(i) - B.at(j).at(i) != 7) vertical = false;
	}

	rep(i, 0, N){
		rep(j, 0, M - 1) if(B.at(i).at(j + 1) - B.at(i).at(j) != 1 || B.at(i).at(j) % 7 == 0) width = false;
	}

	if(vertical && width) Yes;
	else No;
